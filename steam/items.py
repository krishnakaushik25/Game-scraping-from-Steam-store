...
def add_comma(review_summary):
    cleaned_review_summary = ''
    try:
        idx=review_summary.find('%')
        
        cleaned_review_summary =review_summary[:idx-2] + ", " +review_summary[idx-2:]
    except TypeError:
        cleaned_review_summary = 'No reviews'
...
class SteamItem(scrapy.Item):
    ...
    reviews_summary = scrapy.Field(
        input_processor=MapCompose(remove_html,add_comma),
        output_processor=TakeFirst()
    )
